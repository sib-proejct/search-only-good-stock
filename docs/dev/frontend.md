...
Next.js (App Router, TypeScript, Tailwind CSS) 기반 프론트엔드 애플리케이션의 개발 가이드입니다.
...
1. **환경변수 분리**:
   - 백엔드 API 엔드포인트는 `.env.local`의 환경변수(`NEXT_PUBLIC_API_URL=http://localhost:8000`)로 관리합니다.
   - 클라이언트에 노출되면 안 되는 민감한 시크릿 키는 `NEXT_PUBLIC_` 접두사를 붙이지 않습니다.
...
3. **컴포넌트 및 UI 설계**:
   ...
   - 모바일, 태블릿, 데스크톱 환경을 지원하는 반응형 웹 레이아웃을 구현합니다.
